 ...
 */
public class VectorCompass extends AbstractSensor {
    ...
    public VectorCompass(final Context context) {
        super();
    ...
    private void updateBearing(final float newBearing) {
        bearing += AngleMath.deltaAngle(bearing, newBearing);
    ...
    private boolean updateSensor() {
        ...
        // Gravity
        final Vector3 normGravity = accelerometer.getAcceleration().normalize();
        final Vector3 normMagField = magnetometer.getMagneticField().normalize();
        ...
        final float gravityMagnitude = accelerometer.getAcceleration().magnitude();
        final float magneticMagnitude = magnetometer.getMagneticField().magnitude();
